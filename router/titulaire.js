...
router.get("/seances/:id_charge", Secure.verifyToken, async (req, res) => {
  try {
    const seances = await titulaireController.fetchSeancesByChargeId(req.params.id_charge);
    res.status(200).json({
      success: true,
...
router.get("/students/:id_jury/:semestre", Secure.verifyToken, async (req, res) => {
  try {
    const students = await titulaireController.fetStudentByJury({id_jury: req.params.id_jury, semestre: req.params.semestre});
    res.status(200).json({
      success: true,
