...
class Document {
  ...
  _addGradeTable(startRow) {
    ...
    // Colonnes fixes: N°, Nom
    this._addFixedColumns(headerRow1, headerRow2, currentCol);
    currentCol += 2;
    ...
    unites.forEach((unite) => {
      ...
      unite.elements.forEach((element) => {
        ...
        elemCell.font = { name: "Arial", size: 8, bold: true };
        elemCell.alignment = {
          horizontal: "center",
          vertical: "middle",
          wrapText: true,
      ...
      ws.getColumn(currentCol).width = 6;
      currentCol++;
    });

    // Colonnes de synthèse finale
    this._addSummaryColumns(headerRow1, headerRow2, currentCol);
  ...
  /**
   * Ajoute les colonnes fixes (N°, Nom) - SANS MATRICULE
   */
  _addFixedColumns(row1, row2, startCol) {
    const ws = this.worksheet;

    // Numéro
    ws.mergeCells(row1, startCol, row2, startCol);
    ...
    ws.getColumn(startCol).width = 4;

    // Nom complet (SANS MATRICULE)
    ws.mergeCells(row1, startCol + 1, row2, startCol + 1);
    const nameCell = ws.getCell(row1, startCol + 1);
  ...
  /**
   * Ajoute les colonnes de synthèse finale
   */
  _addSummaryColumns(row1, row2, startCol) {
    const ws = this.worksheet;
    ...
    summaryColumns.forEach((col, index) => {
      ws.mergeCells(row1, startCol + index, row2, startCol + index);
  ...
  _addLegend(startRow) {
    ...
    const legendItems = [
      "✓ : Unité validée    ✗ : Unité non validée",
      "Couleurs: Vert ≥16, Jaune 12-15, Orange 10-11, Rouge <10"
    ];
  ...
  genArray() {
    ...
    return students.map((student) => {
      ...
      unites.forEach((unite) => {
        ...
        // Moyenne et décision de l'unité
        row[`${unite.code}_moyenne`] = student.data && student.data[dataIndex] !== undefined ? student.data[dataIndex] : 0;
        dataIndex++;
        row[`${unite.code}_decision`] = student.data && student.data[dataIndex] !== undefined ? student.data[dataIndex] : "NV";
        dataIndex++;
      });
